...
package de.jdellay.wallet.integration.android;
...
public final class BitcoinIntegration
{
	...
	{
		final StringBuilder uri = new StringBuilder("cannacoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Cannacoin application found.\nPlease install Cannacoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=de.jdellay.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://jdellay.github.io/cannacoin-wallet-new/"));
